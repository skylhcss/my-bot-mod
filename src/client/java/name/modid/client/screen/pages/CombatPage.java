...
public class CombatPage extends ConfigPage {
    ...
    protected void buildPage() {
        ...
        ModernSlider attackReach = new ModernSlider(0, 0, 0, 22, 1.0, 10.0, config.attackReachDistance,
            value -> Component.literal(String.format("生存模式攻击距离: %.1f 格", value)),
            value -> { config.attackReachDistance = value; config.save(); });
        ...
        ModernSlider creativeReach = new ModernSlider(0, 0, 0, 22, 1.0, 15.0, config.creativeAttackReachDistance,
            value -> Component.literal(String.format("创造模式攻击距离: %.1f 格", value)),
            value -> { config.creativeAttackReachDistance = value; config.save(); });
        ...
        ModernCheckbox killAura = new ModernCheckbox(0, 0, 0, 22, Component.literal("启用杀戮光环"), config.enableKillAura);
        ...
        ModernSlider auraRange = new ModernSlider(0, 0, 0, 22, 1.0, 10.0, config.killAuraRange,
            value -> Component.literal(String.format("杀戮光环范围: %.1f 格", value)),
            value -> { config.killAuraRange = value; config.save(); });
