...
public class BotsPage extends ConfigPage {
    ...
    public Component getTitle() {
        return Component.literal("假人");
    }

    @Override
    protected void buildPage() {
        SectionCard section = addSection("假人列表（点击打开其设置）");

        // 刷新按钮
        ModernButton refresh = new ModernButton(0, 0, 0, DesignTokens.ROW_HEIGHT,
            Component.literal("刷新列表"), b -> requestList());
        ...
        if (bots.isEmpty()) {
            ModernButton none = new ModernButton(0, 0, 0, DesignTokens.ROW_HEIGHT,
                Component.literal("（当前没有假人）"), b -> requestList());
            none.active = false;
            ...
            for (BotClientData.Entry entry : bots) {
                ...
                ModernButton btn = new ModernButton(0, 0, 0, DesignTokens.ROW_HEIGHT,
                    Component.literal(name + "  ·  " + shortDimension(entry.dimension())),
                    b -> openBotPanel(name));
                section.addItem(btn);
            }
        }
