...
> **v1.3.0 新增**：对着假人**右键**即可打开该假人的设置面板（全局配置界面风格：左侧操作、右侧个人配置），并从中打开背包/末影箱。以下命令为等效入口。

### 打开背包
```
/bot <名字> inventory
```
打开假人背包界面（原版风格，可编辑）：主物品栏 36 格、盔甲 4 格、副手 1 格，可自由放入/取出，改动实时同步到假人。界面左侧显示假人模型，右上 3x3 选择板可设置手持槽位。
...
### 设置游戏模式
```
/bot <名字> gamemode <survival|creative|adventure|spectator>
```
设置假人的游戏模式。

**示例**：
```
/bot Steve gamemode creative
```
