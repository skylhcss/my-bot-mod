...
### 每个假人的设置面板（v1.3.0）
对着假人**右键**可打开该假人的独立设置面板（全局配置界面风格）：
- **左侧（操作）**：打开背包/末影箱、切换游戏模式、设置手持槽位、停止动作、传送、删除
- **右侧（个人配置）**：受到伤害、会饥饿、死亡自动重生、自动跳跃、杀戮光环，均为三态（继承全局 / 强制开 / 强制关），**优先于全局配置**，并随假人驻留保存

这与全局配置界面（B 键）不同；也可在全局配置界面的"假人"标签页点击假人打开其设置面板。

### 全局配置界面折叠与分类（v1.3.0）
全局配置界面的每个分类卡片可点击标题栏折叠/展开；新增"假人"标签页列出所有假人，点击即可打开对应假人的设置面板。
...
- **类型**：布尔值
- **默认值**：`false`
- **说明**：假人死亡后是否自动重生（约 0.5 秒后复活，并回到重生点或创建者位置）。
